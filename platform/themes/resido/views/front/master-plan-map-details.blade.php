  ...
  <div class="row">
    <div class="col-md-12">
      <h1 class="text-center" style="color: white; font-size: 2rem;background: #2eca6a; padding: 40px;">
        <?php echo (isset($masterPlanMapDetails->name)) ? $masterPlanMapDetails->name : ''; ?></h1>
        <br><br>
        <?php 
        if(!empty($masterPlanMapDetails->img))
          $antiqueImage = asset(\App\Models\CityMasterPlan::CITY_MASTER_PLAN_IMAGES_PATH . $masterPlanMapDetails->img);
        else
          $antiqueImage = asset(\App\Models\CityMasterPlan::CITY_MASTER_PLAN_IMAGES_PATH."dp.png");?>
        ...
        <div><?php echo (isset($masterPlanMapDetails->des)) ? $masterPlanMapDetails->des : ''; ?></div>
        <br>
      </div>
    </div>
  </div>
