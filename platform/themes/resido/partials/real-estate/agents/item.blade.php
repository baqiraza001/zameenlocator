<div class="agents-grid">
    <div class="agents-grid-wrap">
        <div class="fr-grid-thumb">
            @if ($account->username)
                <a href="{{ route('public.agent', $account->id) }}">
        ...
        <div class="fr-grid-deatil">
            <div class="fr-grid-deatil-flex">
                <h5 class="fr-can-name">
                    @if ($account->username)
                        <a href="{{ route('public.agent', $account->id) }}">{{ $account->name }}</a>
                    @else
                        {{ $account->name }}
                    @endif
                </h5>
    ...
    <div class="fr-grid-footer">
        ...
        @if ($account->username)
            <div class="fr-grid-footer-flex-right">
                <a href="{{ route('public.agent', $account->id) }}" class="prt-view" tabindex="0">{{ __('View') }}</a>
            </div>
        @endif
    </div>
